...
public class XboxAppDeepLinker {
    ...
    public static boolean appDeeplinkingSupported() {
        boolean isAmazonFireTv = Build.MANUFACTURER.equalsIgnoreCase("Amazon") && Build.MODEL.startsWith(AMAZON_FIRE_TV_MODEL_PREFIX);
    ...
    private static Intent getXboxAppInStoreIntent(Context context, String storeUri, String expectedStorePackage) {
        Intent storeIntent = new Intent("android.intent.action.VIEW", Uri.parse(storeUri + "com.microsoft.xboxone.smartglass"));
        List<ResolveInfo> appsAbleToHandleIntent = context.getPackageManager().queryIntentActivities(storeIntent, 0);
        for (ResolveInfo app : appsAbleToHandleIntent) {
