...
public class ImageUtil {
    ...
    public static URI getTiny2X1(URI url) {
        return formatURI(url, 150, 84);
    }

    public static URI getTiny2X1(String url) {
        return formatString(url, 150, 84);
    }

    public static URI getSmall(URI url) {
        return formatURI(url, SMALL, SMALL);
    }

    public static URI getSmall(String url) {
        URI uri = formatString(url, SMALL, SMALL);
    ...
    public static URI getSmall2X1(URI url) {
        return formatURI(url, 243, 137);
    }

    public static URI getSmall2X1(String url) {
        return formatString(url, 243, 137);
    }

    public static URI getMedium(URI url) {
        return XboxTcuiSdk.getIsTablet() ? formatURI(url, MEDIUM_TABLET, MEDIUM_TABLET) : formatURI(url, MEDIUM_PHONE, MEDIUM_PHONE);
    }

    public static URI getMedium(String url) {
        URI uri;
        if (XboxTcuiSdk.getIsTablet()) {
            uri = formatString(url, MEDIUM_TABLET, MEDIUM_TABLET);
        } else {
            uri = formatString(url, MEDIUM_PHONE, MEDIUM_PHONE);
        }
        if (uri == null && url != null) {
            URI uri2 = createUri(url);
            return uri2;
        }
        return uri;
    }
    ...
    public static URI getLarge(URI url) {
        return XboxTcuiSdk.getIsTablet() ? formatURI(url, LARGE_TABLET, LARGE_TABLET) : formatURI(url, LARGE_PHONE, LARGE_PHONE);
    }
    ...
    private static URI formatString(String url, int width, int height) {
        ...
        return url.contains("format=") ? createUri(url + String.format(resizeFormatterSizeOnly, Integer.valueOf(width), Integer.valueOf(height))) : createUri(url + String.format(resizeFormatter, Integer.valueOf(width), Integer.valueOf(height)));
    }
