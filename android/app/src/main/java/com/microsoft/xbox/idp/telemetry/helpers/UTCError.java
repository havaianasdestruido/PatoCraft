...
public class UTCError {
    ...
    public static void trackFailure(String jobName, boolean isSilent, UTCTelemetry.CallBackSources source, long errorCode) {
        try {
            ...
            ClientError error = new ClientError();
            error.setErrorName("Client Error Type - Failure");
            error.setErrorCode(String.format("%s", Long.valueOf(errorCode)));
            error.setPageName(UTCPageView.getCurrentPage());
            error.setBaseData(UTCCommonDataModel.getCommonData(1, model));
    ...
    public static void trackException(Exception ex, String callingSource) {
        ...
        if (ex != null && callingSource != null) {
            UTCLog.log(String.format("%s:%s", callingSource, ex.getMessage()), new Object[0]);
            ...
            if (stackTrace != null && stackTrace.length > 0) {
                for (int i = 0; i < stackTrace.length && i < 10; i++) {
                    ...
                    if (element != null) {
                        String frame = element.toString();
                        callStack = String.format("%s;%s", callStack, frame);
                    }
    ...
    public static void trackServiceFailure(String errorName, String pageName, HttpError httpError) {
        try {
            ...
            if (httpError != null) {
                ...
                errorCode = String.format("%s", Integer.valueOf(httpError.getErrorCode()));
            }
            ...
            error.setErrorCode(String.format("%s", errorCode));
            error.setBaseData(UTCCommonDataModel.getCommonData(1, model));
