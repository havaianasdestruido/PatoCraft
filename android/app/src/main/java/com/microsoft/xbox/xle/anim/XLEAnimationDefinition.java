    ...
    public Animation getAnimation() {
        ...
        switch (this.property) {
            case Alpha:
                ...
                break;
            case Scale:
                animation = new ScaleAnimation(this.from, this.to, this.from, this.to, this.scaleRelativeToSelf ? 1 : 2, this.pivotX, this.scaleRelativeToSelf ? 1 : 2, this.pivotY);
                break;
            case PositionX:
                animation = new TranslateAnimation(this.fromXType, this.from, this.toXType, this.to, 1, 0.0f, 1, 0.0f);
                break;
            case PositionY:
                animation = new TranslateAnimation(1, 0.0f, 1, 0.0f, this.fromYType, this.from, this.toYType, this.to);
                break;
            case Height:
                int dimId = XLERValueHelper.findDimensionIdByName(this.dimen);
                int height = 0;
                if (dimId >= 0) {
        ...
        if (animation != null) {
            animation.setDuration(this.durationMs);
            animation.setInterpolator(interpolator);
            animation.setStartOffset(this.delayMs);
            return animation;
        }
        return null;
    }
