...
public class Interop {
    ...
    public static void LogCLL(String xuid, String eventName, String eventData) {
        ...
        if (s_cll == null) {
            ...
            s_cll.getCll().log(eventName, eventData, EventEnums.Latency.LatencyRealtime, EventEnums.Persistence.PersistenceCritical, EnumSet.of(EventEnums.Sensitivity.SensitivityNone), 100.0d, ids);
        }
    }
    ...
    public static void InvokeMSA(Context context, int requestCode, boolean isProd, String cid) {
        ...
        if (!isProd) {
            Config.endpointType = Endpoints.Type.DNET;
        }
    ...
    public static void InvokeLatestIntent(Activity activity, Object intentObject) {
        ...
        if (launchIntent != null) {
            if (intentObject instanceof Intent) {
                Intent minecraftIntent = (Intent) intentObject;
                launchIntent.putExtra("com.microsoft.xbox.extra.RELAUNCH_INTENT", minecraftIntent);
                ...
                UTCAdditionalInfoModel model = new UTCAdditionalInfoModel();
                model.addValue(MonitorMessages.PACKAGE, intentPackageName);
    ...
    public static void InvokeAuthFlow(long userPtr, Activity activity, boolean isProd, String signInText) {
        ...
        AuthFlowActivity.setStaticCallbacks(new AuthFlowActivity.StaticCallbacks() { // from class: com.microsoft.xbox.idp.interop.Interop.1
            @Override // com.microsoft.xbox.idp.ui.AuthFlowActivity.StaticCallbacks
            public void onAuthFlowFinished(long userPtr2, AuthFlowScreenStatus authStatus, String cid) {
                AuthFlowActivity.setStaticCallbacks(null);
                Log.d(Interop.TAG, "onAuthFlowFinished: " + authStatus);
                CacheUtil.clearCaches();
                Interop.auth_flow_callback(userPtr2, authStatus.getId(), cid);
            }
        });
        Intent intent = new Intent(activity, (Class<?>) AuthFlowActivity.class);
    ...
    public static void NotificationRegisterCallback(String regId, boolean isCached) {
        Log.i(TAG, "callback");
        try {
            notificiation_registration_callback(regId, isCached);
    ...
    public enum MSAPurpose {
        ...
        public static MSAPurpose fromId(int id) {
    ...
    private static class CllWrapper {
        ...
        public CllWrapper(AndroidCll cll, Context appContext) {
            this.cll = cll;
