...
public class BundleMarshaller {
    ...
    public static DeviceIdentity deviceAccountFromBundle(Bundle bundle) throws BundleMarshallerException {
        try {
            DeviceCredentials deviceCredentials = new DeviceCredentials(bundle.getString(DeviceUsernameKey), bundle.getString(DevicePasswordKey));
            DAToken deviceDAToken = new DAToken(bundle.getString(DeviceDATokenKey), bundle.getByteArray(DeviceSessionKeyBase64Key));
            return new DeviceIdentity(deviceCredentials, bundle.getString(DevicePuidKey), deviceDAToken);
    ...
    public static boolean hasScope(Bundle bundle) {
        String ticketPolicy = bundle.getString(TicketTargetKey, null);
        String ticketTarget = bundle.getString(TicketPolicyKey, null);
        return (ticketPolicy == null || ticketTarget == null) ? false : true;
    }
    ...
    public static Ticket ticketFromBundle(Bundle bundle) throws BundleMarshallerException {
        try {
            ISecurityScope securityScope = scopeFromBundle(bundle);
            return new Ticket(securityScope, new Date(bundle.getLong(TicketExpirationTimeKey)), bundle.getString(TicketValueKey));
        } catch (IllegalArgumentException e) {
    ...
    public static boolean hasError(Bundle bundle) {
        ...
        return (code == 0 && message == null) ? false : true;
    }
    ...
    public static Bundle exceptionToBundle(Exception exception) {
        try {
            ...
            return errorToBundle(SsoServiceError.AccountNotFound, e2.getMessage());
        } catch (MasterRedirectException e3) {
            Bundle bundle = errorToBundle(SsoServiceError.MasterRedirectException, e3.getMessage());
            bundle.putString(RedirectRequestToKey, e3.getRedirectRequestTo());
            return bundle;
        } catch (ClientConfigUpdateNeededException e4) {
            return errorToBundle(SsoServiceError.ClientConfigUpdateNeededException, e4.getMessage());
        } catch (ClientNotAuthorizedException e5) {
            return errorToBundle(SsoServiceError.ClientNotAuthorized, e5.getMessage());
        } catch (UnsupportedClientVersionException e6) {
            return errorToBundle(SsoServiceError.UnsupportedClientVersion, e6.getMessage());
        } catch (StorageException e7) {
            return errorToBundle(SsoServiceError.StorageException, e7.getMessage());
            ...
            return errorToBundle(SsoServiceError.StsException, e9.getMessage());
        } catch (IllegalArgumentException e10) {
            return errorToBundle(SsoServiceError.IllegalArgumentException, e10.getMessage());
        } catch (Exception e11) {
    ...
    public static AuthenticationException exceptionFromBundle(Bundle bundle) throws BundleMarshallerException {
        ...
        String message = bundle.getString(ErrorMessageKey);
        if (code != 0) {
            SsoServiceError error = SsoServiceError.get(code);
            switch (error) {
