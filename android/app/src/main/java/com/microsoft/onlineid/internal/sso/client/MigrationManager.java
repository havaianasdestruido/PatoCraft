...
public class MigrationManager {
    ...
    protected void migrateStorage() {
        ...
        SsoService ssoService = iterator.hasNext() ? iterator.next() : null;
        while (ssoService != null) {
            ...
            if (!ssoServicePackageName.equals(thisAppPackageName)) {
                migrationAttempts++;
                try {
                    Bundle backup = createRetrieveBackupRequest(this._applicationContext).performRequest(ssoService);
                    if (!backup.isEmpty()) {
            ...
            ssoService = iterator.hasNext() ? iterator.next() : null;
        }
