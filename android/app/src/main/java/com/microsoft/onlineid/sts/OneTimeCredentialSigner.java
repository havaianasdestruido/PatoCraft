...
public class OneTimeCredentialSigner {
    ...
    public String generateOneTimeSignedCredential(String appId) {
        ...
        Uri.Builder signedOneTimeCredential = new Uri.Builder().appendQueryParameter(CurrentTimeLabel, Long.toString(this._currentServerTime.getTime() / 1000)).appendQueryParameter(HashAlgorithmLabel, "SHA256").appendQueryParameter(BinaryVersionLabel, AbstractStsRequest.StsBinaryVersion).appendQueryParameter("appid", appId).appendQueryParameter(DATokenLabel, this._daToken.getToken()).appendQueryParameter(NonceLabel, Base64.encodeToString(nonce, 2));
        byte[] derivedKeyBytes = this._sharedKeyGenerator.generateKey(SharedKeyGenerator.KeyPurpose.CredentialSignature, nonce);
        SecretKeySpec derivedKey = new SecretKeySpec(derivedKeyBytes, "HmacSHA256");
        Mac hmac = Cryptography.getInitializedHmacSha256Digester(derivedKey);
        byte[] hash = hmac.doFinal(signedOneTimeCredential.build().getEncodedQuery().getBytes(Strings.Utf8Charset));
        return signedOneTimeCredential.appendQueryParameter(HashLabel, Base64.encodeToString(hash, 2)).build().getEncodedQuery();
    }
}
