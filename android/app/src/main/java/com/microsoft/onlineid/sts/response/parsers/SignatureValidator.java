...
public class SignatureValidator {
    ...
    public void parseSignatureNode(BasePullParser parser) throws XmlPullParserException, StsParseException, IOException {
        new BasePullParser(parser._parser, XmlSigner.SignatureNamespace, "Signature") { // from class: com.microsoft.onlineid.sts.response.parsers.SignatureValidator.1
    ...
    public void parseSignedInfoNode(BasePullParser parser) throws XmlPullParserException, StsParseException, IOException {
        ...
        new BasePullParser(subParser, null, "SignedInfo") { // from class: com.microsoft.onlineid.sts.response.parsers.SignatureValidator.2
            @Override // com.microsoft.onlineid.sts.response.parsers.BasePullParser
    ...
    public void validate() throws StsSignatureException {
        for (Map.Entry<String, String> entry : this._computedDigests.entrySet()) {
            if (this._parsedDigests.containsKey(entry.getKey())) {
                ...
                if (!digest.equals(entry.getValue())) {
                    throw new StsSignatureException(String.format(Locale.US, "Digest mismatch: id=\"%s\", expected=\"%s\", actual=\"%s\"", entry.getKey(), digest, entry.getValue()), new Object[0]);
                }
            }
        }
        ...
        if (!this._signatureValue.equals(signature)) {
            throw new StsSignatureException(String.format(Locale.US, "Signature mismatch: expected=\"%s\", actual=\"%s\"", this._signatureValue, signature), new Object[0]);
        }
    }
}
