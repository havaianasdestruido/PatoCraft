...
public abstract class AbstractStsResponse {
    ...
    public void parse(InputStream stream) throws IOException, StsParseException {
        try {
            ...
            String responseXml = Strings.fromStream(stream, Strings.Utf8Charset);
            String trace = String.format(Locale.US, "%s: %s", getClass().getSimpleName(), responseXml);
            Logger.info(new RedactableResponse(trace));
