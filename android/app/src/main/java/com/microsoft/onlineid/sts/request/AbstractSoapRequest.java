...
public abstract class AbstractSoapRequest<ResponseType extends AbstractSoapResponse> extends AbstractStsRequest<ResponseType> {
    ...
    public Document buildRequest() {
        ...
        Element header = Requests.appendElement(envelope, "s:Header");
        buildSoapHeader(header);
        Element body = Requests.appendElement(envelope, "s:Body");
        buildSoapBody(body);
    ...
    protected void buildSoapHeader(Element header) {
        ...
        Requests.appendElement(header, "wsa:To", getDestination().toString()).setAttribute("s:mustUnderstand", "1");
        ...
        Element security = Requests.appendElement(header, "wsse:Security");
        buildSecurityNode(security);
    }
    ...
    protected final void appendDeviceDAToken(Element parent, DAToken deviceDAToken) {
        String signedDeviceDA = deviceDAToken.getOneTimeSignedCredential(getClockSkewManager().getCurrentServerTime(), MsaAppGuid);
        Element securityTokenElement = Requests.appendElement(parent, "wsse:BinarySecurityToken", signedDeviceDA);
