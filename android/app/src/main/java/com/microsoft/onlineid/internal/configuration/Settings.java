...
public class Settings {
    ...
    public String getSetting(String settingName) {
        String value = null;
        try {
            value = this._storage.readString(settingName, this._defaultSettingsMap.get(settingName));
        } catch (ClassCastException e) {
            Assertion.check(false);
        }
        if (value == null) {
            String value2 = this._defaultSettingsMap.get(settingName);
            return value2;
        }
        return value;
    }
    ...
    public boolean isSettingEnabled(String settingName) {
        return Strings.equalsIgnoreCase(getSetting(settingName), ServerProtocol.DIALOG_RETURN_SCOPES_TRUE);
    }
