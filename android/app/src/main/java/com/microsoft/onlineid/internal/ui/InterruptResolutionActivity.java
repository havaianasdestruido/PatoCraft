...
public class InterruptResolutionActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        WebFlowTelemetryData telemetryData = new WebFlowTelemetryData().setIsWebTelemetryRequested(webTelemetryRequested).setCallingAppPackageName(this._clientPackageName).setCallingAppVersionName(PackageInfoHelper.getAppVersionName(getApplicationContext(), this._clientPackageName)).setWasPrecachingEnabled(false);
        if (savedInstanceState == null) {
            ApiRequest request = WebFlowActivity.getFlowRequest(getApplicationContext(), getIntent().getData(), WebFlowActivity.ActionResolveInterrupt, null, telemetryData).setAccountPuid(this._accountPuid);
            Intent intent = request.asIntent().addFlags(65536);
    ...
    protected void onWebFlowSucceeded(String flowToken) {
        ...
        if (request.isSdkRequest()) {
            ...
            ApiRequest ticketRequest = new TicketManager(getApplicationContext()).createTicketRequest(this._accountPuid, this._requestedScope, this._clientPackageName, this._cobrandingId, this._clientState).setFlowToken(flowToken).setResultReceiver(this._ticketReceiver);
            startService(ticketRequest.asIntent());
            showPendingActivity();
            return;
        }
    ...
    protected void showPendingActivity() {
        Intent intent = new Intent().setClassName(getApplicationContext(), "com.microsoft.onlineid.authenticator.AccountAddPendingActivity").addFlags(65536);
    ...
    private class WebFlowResultHandler extends ActivityResultHandler {
        ...
        @Override // com.microsoft.onlineid.internal.ActivityResultHandler
