...
public class BackupService extends IntentService {
    ...
    public static void pushBackup(Context applicationContext) {
        applicationContext.startService(new Intent(applicationContext, (Class<?>) BackupService.class).setAction(ActionPushBackup));
    }

    public static void pushBackupIfNeeded(Context applicationContext) {
        applicationContext.startService(new Intent(applicationContext, (Class<?>) BackupService.class).setAction(ActionPushBackupIfNeeded));
    }

    protected void retrieveAndPushBackup() {
        Bundle backup;
        List<SsoService> ssoServices = this._serviceFinder.getOrderedSsoServices();
        if (ssoServices.size() > 0 && this._applicationContext.getPackageName().equals(ssoServices.get(0).getPackageName()) && (backup = retrieveBackup(ssoServices.get(0))) != null && !backup.isEmpty()) {
            int slavesToBackup = Math.min(ssoServices.size() - 1, this._config.getInt(ServerConfig.Int.BackupSlaveCount));
            for (int i = 1; i <= slavesToBackup; i++) {
                storeBackup(ssoServices.get(i), backup);
            }
    ...
    protected void retrieveAndPushBackupIfNeeded() {
        long millisSinceLastBackup = System.currentTimeMillis() - this._storage.readLastBackupPushedTime();
        if (millisSinceLastBackup / 1000 >= this._config.getInt(ServerConfig.Int.MaxSecondsBetweenBackups)) {
            retrieveAndPushBackup();
        }
    }
