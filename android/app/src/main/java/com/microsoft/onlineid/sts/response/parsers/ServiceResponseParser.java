...
public class ServiceResponseParser extends BasePullParser {
    ...
    protected void onParse() throws XmlPullParserException, IOException, StsParseException {
        ...
        while (nextStartTagNoThrow()) {
            ...
            if (tag.equals("S:Header")) {
                ...
                XmlPullParser subParser = validator.computeNodeDigest(this);
                ServiceBodyParser parser2 = new ServiceBodyParser(subParser);
                parser2.parse();
    ...
    private void parseAndSaveFromPassport(String pp) throws XmlPullParserException, StsParseException, IOException {
    ...
    private void saveFromPassport(PassportParser parser) throws XmlPullParserException, StsParseException, IOException {
    ...
    private String decryptEncryptedBlob(String encryptedBlob) throws StsParseException {
        byte[] encryptedBlobBytes = TextParsers.parseBase64(encryptedBlob);
        byte[] derivedKey = new SharedKeyGenerator(this._sessionKey).generateKey(SharedKeyGenerator.KeyPurpose.STSDigest, this._encKeyNonce);
        try {
            return new String(Cryptography.decryptWithAesCbcPcs5PaddingCipher(encryptedBlobBytes, derivedKey), Strings.Utf8Charset);
