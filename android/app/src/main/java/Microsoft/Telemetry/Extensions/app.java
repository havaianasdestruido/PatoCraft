...
public class app extends Extension {
    ...
    public void writeNested(ProtocolWriter writer, boolean isBase) throws IOException {
        ...
        if (!canOmitFields || this.expId != Schema.expId_metadata.getDefault_value().getString_value()) {
            writer.writeFieldBegin(BondDataType.BT_STRING, 10, Schema.expId_metadata);
            writer.writeString(this.expId);
            writer.writeFieldEnd();
        } else {
            writer.writeFieldOmitted(BondDataType.BT_STRING, 10, Schema.expId_metadata);
        }
    ...
    protected boolean memberwiseCompareQuick(app that) {
        ...
        return equals && (this.expId == null || this.expId.length() == that.expId.length());
    }

    protected boolean memberwiseCompareDeep(app that) {
        boolean equals = 1 != 0 && super.memberwiseCompareDeep((Extension) that);
        return equals && (this.expId == null || this.expId.equals(that.expId));
    }
}
